...
public class DeleteCategoryController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        listcb.getItems().add("Select Category to Delete");
        for (int i = 0; i < strArray.length; i++) {
            listcb.getItems().add(strArray[i]);
        }
        listcb.getSelectionModel().select("Select Category to Delete");
    }    
    ...
    private void delete(ActionEvent event) throws SQLException {
        Database db = new Database();
        if (listcb.getValue().equals("Select Category to Delete")) {
            lblout.setText("Please Select a Category to Delete");
        } else {
            db.deleteCategory(listcb.getValue().toString());
