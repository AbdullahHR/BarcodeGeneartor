...
public class CategoriesController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        colDesc.setCellValueFactory(new PropertyValueFactory<>("categorydescription"));
        try {
            db.getCategories(oblist);
        } catch (SQLException ex) {
            Logger.getLogger(TypesController.class.getName()).log(Level.SEVERE, null, ex);
        }
        categoriestbl.setItems(oblist);
    }    
    ...
    private void search(ActionEvent event) throws SQLException {
        categoriestbl.getItems().clear();
        Database db = new Database();
        db.searchCategory(oblist, searchtxt.getText());
        colID.setCellValueFactory(new PropertyValueFactory<>("id"));
        colCategory.setCellValueFactory(new PropertyValueFactory<>("category"));
        colDesc.setCellValueFactory(new PropertyValueFactory<>("categorydescription"));
        categoriestbl.setItems(oblist);
